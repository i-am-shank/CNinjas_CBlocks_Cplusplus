...
// Using the logic of last Question -> House Robber  (houses are not in circle)
    ...
    int rob(vector<int>& nums) {
        ...
        */
        ...
        if(n == 1) {
        	...
        	int ans4a = rob_dp_space(nums, n-1); // Ignore last element
        	vector<int> nums2;
        	for(int i=1; i<n; i++) {
        		nums2.push_back(nums[i]);
        	}
        	int ans4b = rob_dp_space(nums2, n-1); // Ignore first element
        	return max(ans4a, ans4b);
        }
    }
};
